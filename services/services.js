	app.service('YamlParse',function(config,$http){
		...
		vm.getschema = function(){
			...
			// return $http.get('RestmarksFull.yaml').then(function(response){
		...
		vm.calc  = function(str){
			...
			for (c=1; c<entities.length; c++){
				...
				// console.log(entity_properties[0])
				var entity_features = entity_properties[0].split("\n");
				// console.log(entity_features)
				var features =[]; 
				var featureTable= [];
	...
	app.service('SignIn',function($rootScope){
		...
		// $rootScope.credentials = btoa('sot1' + ':' + '1234') ;
		// vm.credentials= btoa('sot1' + ':' + '1234');
